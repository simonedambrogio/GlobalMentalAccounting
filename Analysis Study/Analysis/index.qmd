...
For the Jacket, Play, and Plane scenarios (between-subjects in the original studies), we re-estimate each effect using a single, randomly selected condition per participant and compare it with the within-subjects estimate. The code below builds the random split, fits the between-subjects pooled model, and reports the 95% credible interval of the difference (within − between).
